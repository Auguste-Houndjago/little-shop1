    ...
    return (
        ...
        >
            <button 
                type='button' 
                title='smart' 
                className="w-[45px] h-[45px] flex items-center justify-center bg-transparent relative rounded-[7px] cursor-pointer transition-all duration-300 group"
            >
                <span className={`w-full h-full flex items-center justify-center bg-transparent backdrop-blur-[0px] rounded-[10px] transition-all duration-300 border border-[rgba(255,254,254,0.76)] group-hover:backdrop-blur-sm     `}>
                    <svg 
                        width="24" 
                        height="24" 
                        viewBox="0 0 44 43" 
                        ...
                        {[
                            ...
                            <path
                                ...
                                className={`
                                    ...
                                    ${currentPath >= index + 1 ? 'stroke-dashoffset-0 fill-white' : 'stroke-dashoffset-full fill-transparent'}
                                `}
                                style={{
                                    strokeDasharray: '300',
                                    strokeDashoffset: currentPath >= index + 1 ? '0' : '300'
                                }}
                            />
                        ))}
                    </svg>
                ...
                <span 
                    className={`
                        absolute inset-0 bg-[#181818] dark:bg-transparent -z-10 rounded-[10px] 
                        pointer-events-none group-hover:w-3 group-hover:ml-2 
                        transition-all duration-1000
                    `}
                    style={{ 
                        transform: `rotate(${isVisible ? '0' : '55'}deg)`,
                        transformOrigin: 'bottom',
                        transition: `
                            transform 2000ms cubic-bezier(0.4, 0, 0.2, 1),
                            opacity 2500ms cubic-bezier(0.4, 0, 0.2, 1),
                          width 2500ms cubic-bezier(0.4, 0, 0.2, 1) ,  margin-left 1s cubic-bezier(0.4, 0, 0.2, 1);
                        `,
                        opacity: isVisible ? '1' : '0',
                        backgroundColor: isVisible ? 'rgba(24, 24, 24)' : 'rgba(24, 24, 24, 0)'
                    }}
                />
            </button>
        </Link>
