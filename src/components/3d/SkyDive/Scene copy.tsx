...
export default function Scene({ sentences, flavor }: SkyDiveProps) {
  ...
  useGSAP(() => {
    ...
    scrollTl
    .to(can.position, {
      x: 2,  // Descente diagonale vers la droite
      y: SAFE_BOUNDS.yMax - 3,
      duration: 2,
      ease: "power2.inOut"
    })
    .to(can.rotation, {
      z: Math.PI * 0.1,  // Légère rotation pendant la descente
      duration: 1
    }, "<")
    .to(can.position, {
      x: 0,  // Retour au centre
      y: SAFE_BOUNDS.yMax -0.5,  // Légère remontée
      duration: 2,
      ease: "back.out(1.7)"
    })
    .to(can.rotation, {
      z: 0,  // Retour à la rotation normale
      duration: 1
    }, "<");
    ...
    sentences.forEach((_, index) => {
      ...
      // const initialYPos = 0.8; 
      
      // const yPos = index === 0 ? SAFE_BOUNDS.yMax -0.1: SAFE_BOUNDS.yMax - (index * ((SAFE_BOUNDS.yMax - SAFE_BOUNDS.yMin) / (sentences.length - 1)));
